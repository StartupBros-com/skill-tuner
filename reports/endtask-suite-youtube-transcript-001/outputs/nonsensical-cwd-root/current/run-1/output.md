Since `pwd` is `/` (not a real project directory), the file goes to `~/Downloads` per the skill's save-location rule.
...
Final file location: `~/Downloads/Channel_Title.txt` (channel and title pulled from video metadata; falls back to the video ID `ZZ999xxxx1` if metadata is unavailable).